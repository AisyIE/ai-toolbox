...
- 自定义 provider 的 `settingsConfig.autoReviewModelOverride` 是 **provider 级单值**，不是模型映射行字段。应用时后端会把它统一写入 `ai-toolbox-codex-model-catalog.json` 中每条 entry 的 `auto_review_model_override`。仅当用户显式填写非空值时写入；不要默认填自身。若只有 override、没有 model mapping，后端会用 config.toml 的默认 `model`（或 override 本身）种子一条 catalog entry，否则 Codex 读不到 override。官方 provider 不保存/不投影该字段。
