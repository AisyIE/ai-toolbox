...
- Claude Code 的 `[1M]` / `[1m]` 只是客户端上下文能力标记，不是上游模型 ID 的一部分。Gateway 转发前必须从 Claude 请求模型和 provider 映射结果中剥离该后缀；仅剥离 1M 标记不算“模型重映射”，不能因此触发 thinking rectifier 清理 thinking 块。
