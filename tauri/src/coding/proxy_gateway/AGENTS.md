...
- Codex → OpenAI Chat 的多 vendor 推理参数矩阵由 runtime outbound adapter 执行，显式 `data.meta.codexChatReasoning` 优先。该配置可声明 `supportsThinking`、`supportsEffort`、`thinkingParam`、`effortParam`、`effortValueMode` 和 `outputFormat`，用于把 Codex/Responses 风格 `reasoning_effort` 映射到 DeepSeek、OpenRouter、SiliconFlow、DashScope/Qwen、Bailian、Kimi/Moonshot、GLM/Zai、MiniMax、Mimo、StepFun 等 Chat 兼容 provider 的 `thinking` / `enable_thinking` / `reasoning_split` / `reasoning.effort` / `reasoning_effort`。当显式 meta 缺失但 provider meta 上下文存在时，runtime 会按 `providerType` / `apiFormat` / body `model` 做 cc-switch 风格保守 fallback；平台规则（如 OpenRouter、SiliconFlow）优先于模型名规则。没有 provider meta 的纯协议兼容 helper 不能只凭模型名注入 vendor reasoning 参数。新增内置 endpoint 时仍应同步 profile、provider 表单保存逻辑和 runtime 回归测试，fallback 只是防止漏配时退化。
...
- 有损转换检测由 transformer 提供纯检测，runtime 负责策略执行。`ProxyGatewaySettings.lossy_rejection_enabled=true` 时检测到明确不可逆字段应拒绝；请求头 `X-Allow-Lossy: true` 或关闭全局开关时允许 best-effort，但响应必须带 `X-Transformer-Lossy`，不能静默丢数据。当前检测按源协议覆盖 OpenAI Chat、OpenAI Responses、Anthropic Messages、Gemini Native 的明确不可逆字段：Chat audio/modalities/未知 content part/parallel tool calls，Responses code/computer/local shell/search/MCP/image generation/compaction item 和 tool，Anthropic native/server tool definition 与 block，Gemini native tools、Gemini-only generation config、cachedContent/safetySettings 和非图片媒体。新增不可逆字段时必须先补 `LossyIssue` 和测试。
