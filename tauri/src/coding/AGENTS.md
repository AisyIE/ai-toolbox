...
- 「在用户终端里非阻塞启动常驻 CLI」（如 `hermes dashboard` / `openclaw gateway`）走 `cmd /C start ... cmd /K "<cli>"`（macOS 用 osascript、linux 用 `*-terminal -e`）。`spawn()` 在这种分层派生下**无论子进程是否真正起来都会返回 Ok**——CLI 不存在或启动即报错时，新开终端打印错误后关闭，但 Tauri 命令已返回 Ok，前端会弹误导性成功 toast。启动前必须先用 `cli_resolver::cli_resolved_on_path(name)` 验证 CLI 在 PATH 中可解析，找不到就返回 `local_cli_missing_hint(name)` 的明确错误，由前端引导，不要等 spawn 后再去探活。
